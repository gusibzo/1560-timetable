...
text = re.sub(r"Rev\.1[789]", "Rev.20", text)
for old, new in {
...
css = r'''
/* Rev20: elapsed counter, red Sunday markers, and count-only summary */
.seg[data-day="sunday"]{font-size:12px;letter-spacing:-.5px}
.chips .chip:nth-child(1),.chips .chip:nth-child(2){display:none!important}
.chips .chip:nth-child(3){flex:1 1 100%;width:100%}
...
/* Rev20 elapsed-time counter: starts when the timetable time arrives. */
function rev20Events(day){
 const events=[];
...
function rev20SyncCounter(){
 ...
 const now=new Date();
 const events=rev20Events(state.day);
 let event=null;
 for(const item of events){
  if(rev20TargetMs(item,now)<=now.getTime())event=item;else break;
 }
 if(!event)return;
 const row=rowsEl.children[event.ri],td=row&&row.children[event.ci+1];
 if(!td)return;
 const counter=rev20CounterNode();
 if(event.kind==="ext"){
  const ext=td.querySelector(".ext");if(!ext)return;ext.classList.add("next-ext");ext.appendChild(counter);
 }else{
  td.classList.add("next");rev20EnsureTimeSpan(td,event.time);td.appendChild(counter);
 }
 const diff=Math.max(0,Date.now()-rev20TargetMs(event,now));
...
setInterval(rev20SyncCounter,1000);
setTimeout(rev20SyncCounter,0);
'''
...
if sw.exists():
    ...
    sw_text = re.sub(r'const CACHE_NAME="[^"]+";', 'const CACHE_NAME="1560-timetable-rev20-v1";', sw_text)
